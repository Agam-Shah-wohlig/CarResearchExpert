...
car_subagent_1 = LlmAgent(
    name="car_research_subagent_1",
    model=GEMINI_MODEL,
...
1. Take the structured car name provided by the guide_agent:
   ...
   - Your job is to focus exclusively on: cars_name["car_1"].
2. Use the Google Search tool to gather authoritative, up-to-date information 
   only about car_1 and its top variants (e.g., trims, editions, engine options).
...
Important rules:
- Do **not** research, mention, or reference car_2 in any way.
